function [W3] = solveW3(h_k3,P_0,R,N_t,C,K,lambda_c,lambda_s,f_k3,f_K1_3,sigma_k2,sigma_s2,a_c,a_s,alpha_c,alpha_s,epsilon)
%%%%%  参数设置
  ...
  a_c_tilde = zeros(N_t+1,C);
   ...
   for i = 1:K
        ...
        % 定义 B_k (23)
        B(:,:,i) = [abs(f_k3(i))^2 * h_k3(:,i) * h_k3(:,i)',  zeros(N_t,1);  
                    2 * f_k3(i).' * h_k3(:,i)', 0];
   end
    for c = 1:C
       %定义 E_c
     ...
     %定义 D
       D = [abs(f_K1_3)^2 * (alpha_s)^2 * a_s * (a_s)',  zeros(N_t,1);  
           2 * conj(f_K1_3) * (a_s)', 0];
 ...
 variable c_s  nonnegative
   ...
   %minimize(square_pos(norm(sum(W_jian(1:N_t, 1:N_t), 3) - R, 'fro')))
    ...
    %%(21)
for i = 1:K
    real(trace(B(:,:,i) * W_jian(:,:,i)) - abs(f_k3(i))^2 * (sigma_k2 + trace(A(:,:,i) * sum(W_jian,3))))  >=  c_k(i);%+ 1/6 * log(2)^3 * pow_pos(c_k(i), 3) + 1/24 * log(2)^4 * pow_pos(c_k(i), 4) + 1/120 * log(2)^5 * pow_pos(c_k(i), 5);   
end
    %%(22)
   real(trace(D * sum(W_jian,3)) - abs(f_K1_3)^2 * (sigma_s2 + trace(sum(E,3) * sum(W_jian,3)))) >= c_s; %+ 1/6 * log(2)^3 * pow_pos(c_s, 3) + 1/24 * log(2)^4 * pow_pos(c_s, 4) + 1/120 * log(2)^5 * pow_pos(c_s, 5);%%
